...
mypath = "output/braindata210118/Signal/"
filecase = "signal1_imf_"
...
with open('Signal1_2018.csv', newline='') as f:
    reader = csv.reader(f)
    for row in reader:
        km = len(row)
        k = 0
...
#mask = mmode + mfilt +mmode4
    ...
    mfilt = 0.0*np.sin(2 * np.pi * 20 * t1)
    mask1 = mfilt
    mask2 = mfilt
    mask3 = mfilt
    mask4 = mfilt
